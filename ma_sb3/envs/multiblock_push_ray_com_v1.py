...
class MultiBlockPushRayCom(MujocoEnv, BaseMAEnv):
    ...
    def __init__(self, num_cubes=2, num_blocks=1, agent_speed=0.5, nrays=5, span_angle_degrees=180, block_density=2000, **kwargs):
        ...
        for i in range(num_cubes):
            # Observation space
            observation_space = gym.spaces.Box(low=np.array([0, 0, 0, -1]*self.nrays + [-2]*2, dtype=np.float32), high=np.array([1, 1, 1, 1]*self.nrays + [+2]*2, dtype=np.float32), shape=(4*self.nrays+2,))
            # Action space
            action_space = gym.spaces.Box(low=np.array([-1]*3, dtype=np.float32), high=np.array([+1]*3 , dtype=np.float32), shape=(3,))
        ...
        self.agent_comm_messages = {mujoco_cube_id: 0 for mujoco_cube_id in self.mujoco_cube_ids.values()}
    ...
    def get_observation(self, agent_id):
        ...
        block_obs = np.zeros((self.nrays, 4), dtype=np.float32)
        for i, detected_body_id in enumerate(detected_body_ids):
            ...
            # If the detected body is a cube different from the agent's cube
                ...
                block_obs[i][2] = normalized_distances[i]
                block_obs[i][3] = self.agent_comm_messages[detected_body_id] # Communication message from the agent
            # For debugging
    ...
    def step_agent(self, agent_id:int, action: ActType) -> tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        self.agent_comm_messages[mujoco_cube_id] = action[2] # To be displayed and used by other agents
